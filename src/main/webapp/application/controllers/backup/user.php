				 ...
				 $ticket_numbers = json_decode($ticket_data->ticket_numbers);
				   ...
				   if(count($arr)>0)
				   {
					   ...
					   $all_data = $this->usermodel->getTransactionDetails($str);
				   				  ...
				   				  $user=$this->usermodel->getLoggedinUserData();$sales=array();$rs=array();
				  for($i=0;$i<count($all_data);$i++)
				  {
					  $win=0;
					   $win_amount =$this->usermodel->getWinAmount($user->mid,$all_data[$i]->bate_series_number,$all_data[$i]->valid_till_date,$all_data[$i]->game_id_custom);
					//  $win_amount =$this->usermodel->getWinAmount($user->mid,$all_data[$i]->bate_series_number,$all_data[$i]->valid_till_date);
					
					 if(count($win_amount)>0)
					 {
					    $win=$win_amount[0]->win_amount;
					 }
					
					  $sales_rep=array(
							"bate_series_number"=> $all_data[$i]->bate_series_number,
							"bate_amount_by_user" => $all_data[$i]->bate_amount_by_user,
							"win_amount" =>$win
							);
							array_push($sales, $sales_rep);
				  }
				  
				
				
				 $data['tran_data1']=$sales;
				  ...
				  for($i=0;$i<count($all_data);$i++)
				  {
					 ...
					 if(count($win_amount)>0)
					 {
					  ...
					  $sales_rep=array(
							"bate_series_number"=> $all_data[$i]->bate_series_number,
							"bate_amount_by_user" => $all_data[$i]->bate_amount_by_user,
							"bate_series_column" => $all_data[$i]->bate_series_column,
							"win_amount" =>$win
							);
							array_push($sales, $sales_rep);
				  }
...
$data["damount"] = $this->usermodel->getAllLastTwoDaysTickets($config["per_page"], $page-1);
	...
	public function cancel_ticket()
	{
        ...
        foreach ($all_data as  $vv) {
			...
			else if (date("Y-m-d") == $vv->valid_till_date) 
			{
        ...
        if ($timeout) {
          ...
          redirect("user/ToCancelTickets?msg=Already timeout.Ticket cannot be cancelled");
        }
				 ...
				 if (isset($_POST)) {
					if (!empty($_POST['dt'])) {
						...
						for($i=0;$i<count($usertickets);$i++){
							 ...
							 //$sql="select *,sum(win_amount) win_amount1 from tb_game_result where winning_number='".$usertickets[$i]->bate_series_number."' and game='".$usertickets[$i]->game_id_custom."' and date like ('%".$usertickets[$i]->valid_till_date."%') group by  winning_number";
							
							
							
							// $sql="select *,sum(win_amount) win_amount1 from tb_game_result where mid='".$user->mid."' and game='".$usertickets[$i]->game_id_custom."' and date like ('%".$usertickets[$i]->valid_till_date."%') group by winning_number";
							 
							  $sql="select *,sum(win_amount) win_amount1 from tb_game_result where mid='".$user->mid."' and game='".$usertickets[$i]->game_id_custom."' and DATE_FORMAT(date,'%Y-%m-%d')='".$usertickets[$i]->valid_till_date."' group by game";
						   ...
						   //echo "<br><br>------------------<br><br>";
							...
							/*	$data=array(
							    ...
							    $data=array(
								"bated_amount"=>$usertickets[$i]->points,
								"win_amount" =>$rs[0]->win_amount1,
								"date" => $usertickets[$i]->date_time,
								"game"=> $dt
								); 
				...
				if (!empty($_POST['date_form']) && !empty($_POST['date_to'])) {
						...
						if(count($user_tickets)>0)
						{
							for($i=0;$i<count($user_tickets);$i++)
							{
								...
								{
									$sql="select SUM( win_amount ) win_amt from tb_game_result where mid='".$user->mid."'  and date like ('%".$usertickets[$j]->valid_till_date."%')";
									 $data=$this->db->query($sql);
									 $rs=$data->result();
									 $win=$win+$rs['0']->win_amt;
						...
						else
						{
							...
							array_push($salesReposrtUserResults, $value1);
						}
						$data['sales_report']=$salesReposrtUserResults;
